...
class SoulState:
    ...
    is_responding: bool = False  # LLM is generating (set by BOT_RESPONSE clear)
    bot_is_speaking: bool = False  # TTS audio is actually playing
...
class SoulLoop:
    ...
    def __init__(
        ...
        logger.info(
            f"[STATE] SoulLoop initialized: poll={config.poll_interval_ms}ms, emotion_inference={config.emotion_inference_interval_ms}ms"
        )
    ...
    def _apply_personality_to_config(self):
        ...
        # NOTE: breathing_enabled was removed from SoulConfig — breathing is
        # now handled exclusively by MovementManager.BreathingMove.

        if "micro_movements_enabled" in idle_config:
            self.config.idle_micro_movements_enabled = idle_config[
                "micro_movements_enabled"
            ]
            if self.config.log_decisions:
                soul_log.log_personality_applied(
                    "idle_micro_movements_enabled",
                    idle_config["micro_movements_enabled"],
                )
        ...
        if "scanning_enabled" in idle_config:
            ...
            if self.config.log_decisions:
                soul_log.log_personality_applied(
                    "idle_scanning_enabled", idle_config["scanning_enabled"]
                )
    ...
    async def start(self):
        ...
        soul_log.log_loop_start(
            config_summary={
                ...
                "breathing": "external (BreathingMove)",
    ...
    async def _run_loop_inner(self, poll_interval: float):
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                # Generate and apply idle behaviors (suppress during active TTS playback)
                if not self._state.bot_is_speaking:
                ...
                if idle_duration > 5.0:
                    await self._event_queue.push(
                        IdleTickEvent(idle_duration_s=idle_duration)
                    )
                ...
                if self.config.log_status_interval_s > 0:
                    now = time.time()
                    if (
                        now - self._last_status_log_time
                        >= self.config.log_status_interval_s
                    ):
    ...
    async def _check_personality_reload(self):
        ...
        now = time.time()
        if (
            now - self._last_personality_reload
            < self.config.personality.reload_interval_s
        ):
            return
    ...
    async def _handle_event(self, event: SoulEvent):
        ...
        if event.event_type == EventType.USER_SPEAKING:
            ...
            if self.config.log_events and old_state != "speaking":
                soul_log.log_state_change(
                    "user_state", old_state, "speaking", reason="USER_SPEAKING event"
                )
            ...
            if self.config.log_events and old_state != "waiting":
                soul_log.log_state_change(
                    "user_state", old_state, "waiting", reason="USER_SILENT event"
                )
            ...
            if self.config.log_events and was_responding:
                soul_log.log_state_change(
                    "is_responding", True, False, reason="BOT_RESPONSE complete"
                )
            ...
            if self.config.log_events and not was_responding:
                soul_log.log_state_change(
                    "is_responding", False, True, reason="BOT_STREAMING started"
                )

        elif event.event_type == EventType.BOT_SPEAKING_STARTED:
            was_speaking = self._state.bot_is_speaking
            self._state.bot_is_speaking = True
            self._state.is_responding = True

            if self.config.log_events and not was_speaking:
                soul_log.log_state_change(
                    "bot_is_speaking",
                    False,
                    True,
                    reason="BOT_SPEAKING_STARTED (TTS playing)",
                )

        elif event.event_type == EventType.BOT_SPEAKING_STOPPED:
            was_speaking = self._state.bot_is_speaking
            self._state.bot_is_speaking = False
            self._state.is_responding = False

            if self.config.log_events and was_speaking:
                soul_log.log_state_change(
                    "bot_is_speaking",
                    True,
                    False,
                    reason="BOT_SPEAKING_STOPPED (TTS done)",
                )
            ...
            # Suppress scanning while a face is being tracked to prevent
            # the scan pattern from fighting with face-following offsets.
            # Face tracking weight is controlled by MovementManager mode system.
            if not was_detected:
                self._idle_generator.suppress_scanning(True)
                if self.config.log_decisions:
                    soul_log.log_decision(
                        decision_point="face_tracking",
                        decision="suppress_scanning",
                        reason="face_detected - scanning suppressed to avoid conflict",
                    )

            if self.config.log_events and not was_detected:
                soul_log.log_state_change(
                    "face_detected", False, True, reason="FACE_DETECTED event"
                )
            ...
            # Re-enable scanning now that face is gone
            if was_detected:
                self._idle_generator.suppress_scanning(False)
                if self.config.log_decisions:
                    soul_log.log_decision(
                        decision_point="face_tracking",
                        decision="allow_scanning",
                        reason="face_lost - scanning allowed again",
                    )

            if self.config.log_events and was_detected:
                soul_log.log_state_change(
                    "face_detected", True, False, reason="FACE_LOST event"
                )
    ...
    async def _update_emotion(self):
        ...
        if should_infer:
            ...
            if (
                result.emotion != self._state.current_emotion
                or abs(result.intensity - self._state.emotion_intensity) > 0.1
            ):
    ...
    def on_user_message(self, message: str):
        ...
        asyncio.create_task(self._event_queue.push(UserMessageEvent(message=message)))

    def on_bot_response(self, response: str):
        """Call when bot completes a response."""
        asyncio.create_task(self._event_queue.push(BotResponseEvent(response=response)))
    ...
    def on_bot_speaking_started(self):
        """Call when TTS audio actually starts playing."""
        asyncio.create_task(self._event_queue.push(BotSpeakingStartedEvent()))

    def on_bot_speaking_stopped(self):
        """Call when TTS audio finishes playing."""
        asyncio.create_task(self._event_queue.push(BotSpeakingStoppedEvent()))
    ...
    def on_face_lost(self, face_id: Optional[str] = None):
        ...
        asyncio.create_task(self._event_queue.push(FaceLostEvent(face_id=face_id)))
