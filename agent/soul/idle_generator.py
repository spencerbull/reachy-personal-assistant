...
- Micro-movements: Tiny random head adjustments
...
class IdleState:
    ...
    breathing_phase: float = 0.0  # Current phase in breathing cycle (0-2π)
...
class IdleGenerator:
    ...
    def update(self, dt: Optional[float] = None) -> Pose:
        ...
        # Generate breathing
        if self.config.idle_breathing_enabled:
            breathing = self._generate_breathing(dt)
            overlay.head_z = breathing.head_z
            overlay.head_pitch = breathing.head_pitch
    ...
    def _generate_breathing(self, dt: float) -> Pose:
        """
        Generate breathing motion.
        
        Uses a modified sine wave with longer exhale for natural feel.
        """
        # Update breathing phase
        self._state.breathing_phase += dt * self.config.idle_breathing_frequency_hz * 2 * math.pi
        self._state.breathing_phase %= 2 * math.pi
        
        phase = self._state.breathing_phase
        
        # Modified breathing curve: faster inhale, slower exhale
        # Using sine with asymmetric modification
        if phase < math.pi:
            # Inhale (faster)
            t = phase / math.pi
            breath = math.sin(t * math.pi / 2)  # 0 to 1
        else:
            # Exhale (slower)
            t = (phase - math.pi) / math.pi
            breath = math.cos(t * math.pi / 2)  # 1 to 0
        
        # Apply amplitude
        amplitude = self.config.idle_breathing_amplitude
        
        # Breathing causes slight vertical movement and head tilt
        breathing_pose = Pose(
            head_z=breath * amplitude,
            head_pitch=breath * 0.01,  # Very subtle forward/back
        )
        
        return breathing_pose
