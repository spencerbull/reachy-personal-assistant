...
class EventType(Enum):
    ...
    BOT_SPEAKING_STARTED = "bot_speaking_started"  # TTS audio actually playing
    BOT_SPEAKING_STOPPED = "bot_speaking_stopped"  # TTS audio finished
...
class BotResponseEvent(SoulEvent):
    ...
    def __str__(self) -> str:
        preview = (
            self.response[:50] + "..." if len(self.response) > 50 else self.response
        )
...
class BotStreamingEvent(SoulEvent):
    ...
    def __str__(self) -> str:
        return (
            f"BotStreaming: +'{self.token}' (total: {len(self.accumulated_text)} chars)"
        )


@dataclass
class BotSpeakingStartedEvent(SoulEvent):
    """Bot TTS audio has started playing (actual speech output)."""

    event_type: EventType = field(default=EventType.BOT_SPEAKING_STARTED)

    def __str__(self) -> str:
        return "BotSpeakingStarted"


@dataclass
class BotSpeakingStoppedEvent(SoulEvent):
    """Bot TTS audio has finished playing."""

    event_type: EventType = field(default=EventType.BOT_SPEAKING_STOPPED)

    def __str__(self) -> str:
        return "BotSpeakingStopped"
...
class SoundDetectedEvent(SoulEvent):
    ...
    direction: Optional[str] = (
        None  # "left", "right", "front", "behind" if determinable
    )
